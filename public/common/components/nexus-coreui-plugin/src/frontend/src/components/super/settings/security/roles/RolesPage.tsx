...
export function RolesPage() {
  ...
  // Load role details when in edit mode
  useEffect(() => {
    if (routeState.viewMode === 'edit' && routeState.roleId) {
  ...
  const handleSelectRole = useCallback((roleId: string) => {
    navigateTo(`${BASE_PATH}/${encodeURIComponent(roleId)}`);
  }, []);
  ...
  const headerProps = useMemo(() => {
    switch (routeState.viewMode) {
      ...
      case 'create':
        const createSteps = ['Setup', 'Privileges', 'Contained Roles'];
        return {
          icon: Shield,
          title: 'Create Role',
...
/>
        ...
        {(routeState.viewMode === 'create' || (routeState.viewMode === 'edit' && role)) && (
          <WizardForm
            ...
            dirty={false}
            canAdvance={internalWizardStep === 0 || internalWizardStep === 2 ? isRoleFormValid : true}
            loading={loading && internalWizardStep === 2}

...
describe('searchMachine', () => {
  ...
  describe('format classification', () => {
    const EXPECTED_SIMPLE = ['all', 'apt', 'cargo', 'cocoapods', 'conda', 'go', 'helm', 'huggingface', 'r', 'raw'];
    const EXPECTED_CUSTOM = ['composer', 'conan', 'docker', 'gitlfs', 'maven', 'npm', 'nuget', 'p2', 'pypi', 'rubygems', 'swift', 'terraform', 'yum'];
    ...
    it('covers all 24 formats', () => {
      expect(ALL_FORMATS).toHaveLength(23); // 23 formats (22 specific + 'all')
    });
  });

...
const ALL_FORMATS = [
  'all', 'apt', 'cargo', 'cocoapods', 'composer', 'conan', 'conda',
  'docker', 'gitlfs', 'go', 'helm', 'huggingface', 'maven', 'npm',
  'nuget', 'p2', 'pypi', 'r', 'raw', 'rubygems', 'swift', 'terraform', 'yum'
] as const;
...
describe('searchFilters', () => {
  describe('FORMATS - All 23 Formats', () => {
    it('defines exactly 23 formats', () => {
      expect(Object.keys(FORMATS).length).toBe(23);
    });
  ...
  describe('FORMAT_FILTERS - All 23 Formats', () => {
    it('defines filters for exactly 23 formats', () => {
      expect(Object.keys(FORMAT_FILTERS).length).toBe(23);
    });
    ...
    describe('Formats with no custom filters', () => {
      const formatsWithOnlyRepository = ['all', 'apt', 'cargo', 'cocoapods', 'conda', 'go', 'helm', 'huggingface', 'r', 'raw'];

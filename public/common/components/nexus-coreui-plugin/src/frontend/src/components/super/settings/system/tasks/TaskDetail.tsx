...
import { Trash2, Play, Square, FileText, Settings, Calendar, History, Edit3 } from 'lucide-react';
          ...
          <Box className="task-detail__tab-content">
            ...
            <Tabs.Content value="summary">
              <Box className="task-detail__summary-grid">
                <SummaryItem label="Status" value={displayTask.status} />
                <SummaryItem label="Last Result" value={displayTask.lastRunResult || '-'} />
                <SummaryItem
                  label="Schedule"
                  value={displayTask.schedule || '-'}
                  onClick={canEdit ? () => setActiveTab('schedule') : undefined}
                />
                <SummaryItem label="Next Run" value={displayTask.nextRun ? formatDate(displayTask.nextRun) : '-'} />
                <SummaryItem label="Last Run" value={displayTask.lastRun ? formatDate(displayTask.lastRun) : 'Never'} />
                <SummaryItem
                  label="Alert Email"
                ...
                {displayTask.statusDescription && (
                  <SummaryItem label="Description" value={displayTask.statusDescription} />
                )}
...
function SummaryItem({ label, value, onClick }: { label: string; value: string; onClick?: () => void }) {
  return (
    <Box

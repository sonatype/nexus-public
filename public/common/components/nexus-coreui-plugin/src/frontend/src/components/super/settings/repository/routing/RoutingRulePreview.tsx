  ...
  return (
    <Box className="routing-rule-preview__node">
      ...
      >
        ...
        {/* Rule name */}
        {item.rule && (
          <Text size="1" className="routing-rule-preview__rule">
            {item.rule}
          </Text>
        )}
      ...
      {expanded && hasChildren && (
        <Box className="routing-rule-preview__children">
          {item.children!.map((child, index) => (
            <TreeNode key={`${child.repository}-${index}`} item={child} level={level + 1} />
          ))}
        </Box>
      )}
    </Box>
...
export function RoutingRulePreview({ onClose }: RoutingRulePreviewProps) {
  ...
  const { fetchRoutingRulesPreview } = useRoutingRulesApi();
          ...
          {previewData.children.length > 0 ? (
            <Box className="routing-rule-preview__tree">
              {previewData.children.map((item, index) => (
                <TreeNode key={`${item.repository}-${index}`} item={item} level={0} />
              ))}
            </Box>
          ) : (

...
describe('EntityTable', () => {
  ...
  it('applies data-testid to each row when getRowTestId is provided', () => {
    renderWithTheme(
      <EntityTable
        data={testData}
        columns={columns}
        getRowKey={(item) => item.id}
        getRowTestId={(item) => `repo-row-${item.name}`}
      />
    );

    expect(screen.getByTestId('repo-row-maven-central')).toBeInTheDocument();
    expect(screen.getByTestId('repo-row-npm-hosted')).toBeInTheDocument();
    expect(screen.getByTestId('repo-row-docker-hub')).toBeInTheDocument();
  });

  it('does not add data-testid to rows when getRowTestId is not provided', () => {
    const { container } = renderWithTheme(
      <EntityTable
        data={testData}
        columns={columns}
    ...
    const rows = container.querySelectorAll('tr[data-testid]');

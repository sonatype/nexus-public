...
describe('RealmsPage', () => {
  ...
  // TODO: Implement validation for empty active realms in RealmsPage component
  it.skip('shows validation error when no realms are active', async () => {
    render(<RealmsPage />, { wrapper: TestWrapper });
    ...
    const removeAllButton = screen.getByLabelText('Remove all realms');
    fireEvent.click(removeAllButton);

    // Validation error should be shown
    await waitFor(() => {
      expect(screen.getByText(/At least one realm must be selected/)).toBeInTheDocument();
    });
  ...
  // TODO: Implement SSO validation in RealmsPage component
  it.skip('shows validation error when multiple SSO realms are active', async () => {
    const realmsWithSso = [
      { id: 'NexusAuthenticatingRealm', name: 'Local Authenticating Realm' },
      { id: 'NexusAuthorizingRealm', name: 'Local Authorizing Realm' },
      { id: 'SamlRealm', name: 'SAML Realm' },
      { id: 'OAuth2Realm', name: 'OAuth2 Realm' },
    ];
    ...
    mockedUseRealmsApi.mockReturnValue({
      ...
      error: null,
      setError: mockSetError,
      fetchAvailableRealms: mockFetchAvailableRealms.mockResolvedValue(realmsWithSso),
      fetchActiveRealmIds: mockFetchActiveRealmIds.mockResolvedValue(['NexusAuthenticatingRealm', 'SamlRealm']),
      updateActiveRealms: mockUpdateActiveRealms,
    });
    ...
    // Add OAuth2 realm (which creates SSO conflict)
    const oauth2Item = screen.getByText('OAuth2 Realm').closest('[role="option"]');
    fireEvent.doubleClick(oauth2Item!);

    // Validation error should be shown
    await waitFor(() => {
      expect(screen.getByText(/Only one SSO realm can be enabled at a time/)).toBeInTheDocument();
    });
  ...
  // TODO: Integrate LocalRealmRemovalModal into RealmsPage component
  it.skip('shows local realm removal modal when removing NexusAuthenticatingRealm', async () => {
    render(<RealmsPage />, { wrapper: TestWrapper });
    ...
    // Select and remove Local Authenticating Realm
    const authItem = screen.getByText('Local Authenticating Realm').closest('[role="option"]');
    fireEvent.click(authItem!);

    const removeButton = screen.getByLabelText('Remove selected realms');
    fireEvent.click(removeButton);

    // Try to save - should show modal
    const saveButton = screen.getByRole('button', { name: 'Save' });
    await waitFor(() => {
      expect(saveButton).not.toBeDisabled();
    });
  ...
  // TODO: Integrate LocalRealmRemovalModal into RealmsPage component
  it.skip('saves after confirming local realm removal', async () => {
    render(<RealmsPage />, { wrapper: TestWrapper });
    ...
    // Should save
    await waitFor(() => {
      expect(mockUpdateActiveRealms).toHaveBeenCalledWith(['NexusAuthorizingRealm']);
    });
  });

  // TODO: Integrate LocalRealmRemovalModal into RealmsPage component
  it.skip('closes modal without saving when cancelled', async () => {
    render(<RealmsPage />, { wrapper: TestWrapper });
    ...
    expect(mockUpdateActiveRealms).not.toHaveBeenCalled();
  });

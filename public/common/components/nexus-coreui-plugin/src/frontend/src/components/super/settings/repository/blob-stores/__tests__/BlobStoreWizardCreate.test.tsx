...
async function goToBasicConfigStep() {
  ...
  const nextButton = screen.getByRole('button', { name: /Next: Basic Config/i });
...
describe('BlobStoreWizardCreate', () => {
  ...
  it('has all 4 wizard steps in indicator', () => {
    renderWithTheme(<BlobStoreWizardCreate onBack={jest.fn()} />);
  ...
  it('shows Next button on step 0 (type selection)', () => {
    renderWithTheme(<BlobStoreWizardCreate onBack={jest.fn()} />);
  ...
  it('disables Next on Basic Config when path is root slash', async () => {
    ...
    const nextButton = screen.getByRole('button', { name: /Next: Credentials/i });

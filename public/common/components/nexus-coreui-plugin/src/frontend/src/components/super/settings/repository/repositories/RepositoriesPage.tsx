...
import { PageHeader, type PageHeaderProps } from '../../../../shared';
import React, { useState, useEffect, useCallback, useMemo, useRef } from 'react';
import { Box, Button } from '@radix-ui/themes';
import { Database, Plus, ArrowLeft } from 'lucide-react';
import { ExtJS } from '@sonatype/nexus-ui-plugin';

import { PageHeader, ErrorState, useToast } from '../../../../shared';
...
} from '@/components/shared/security/useFirewallEnable';
import { useRepositoriesApi } from './useRepositoriesApi';
import { Repository, Recipe, RepositoryFormData } from './types';
...
export function RepositoriesPage() {
  ...
  const selectedRecipe = useMemo<Recipe | null>(() => {
    if (routeState.format && routeState.type) {
      return { format: routeState.format, type: routeState.type as any, name: `${routeState.format}-${routeState.type}` };
    }
    return null;
  ...
  const editRecipe = useMemo<Recipe | null>(() => {
    if (repository) {
      return { format: repository.format, type: repository.type as any, name: `${repository.format}-${repository.type}` };
    }
    return null;
  }, [repository]);
  ...
  const handleWizardStepChange = useCallback((step: number) => {
    ...
    if (step === 0) {
      ...
      setCanAdvanceFromStep1(!!pendingRecipe?.type);
      navigateTo(`${BASE_PATH}/create`);
    } else if (step === 2) {
  ...
  const handleSave = useCallback(
    ...
    [selectedRecipe, routeState.viewMode, postCreateStep, firewallChoice, rhcChoice, finishCreate, enableHealthCheck, toast]
  );
  ...
  const getHeaderProps = () => {
    switch (routeState.viewMode) {
      ...
      case 'create': return { icon: selectedRecipe ? <FormatIcon format={selectedRecipe.format} type={selectedRecipe.type} size={24} /> : Database, title: `Create ${(selectedRecipe?.format || '').replace(/2$/, '')} (${selectedRecipe?.type || ''}) Repository`, description: 'Configure settings', actions: <Button variant="ghost" onClick={handleBackToTypeSelect}><ArrowLeft size={16} /> Back</Button> };

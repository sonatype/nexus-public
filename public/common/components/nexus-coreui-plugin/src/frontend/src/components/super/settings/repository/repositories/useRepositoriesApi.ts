...
function buildRepositoryConfig(data: RepositoryFormData): Record<string, unknown> {
  ...
  if (data.storage) {
    config.storage = {
      blobStoreName: data.storage.blobStoreName,
  ...
  if ((data.format === 'apt' && data.type === 'hosted') || data.aptSigning) {
    config.aptSigning = {
      keypair: data.aptSigning?.keypair || '',
      passphrase: data.aptSigning?.passphrase || '',
    };
  }

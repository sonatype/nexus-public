...
describe('CapabilitiesList', () => {
  ...
  describe('enriched columns (bug o7gp)', () => {
    ...
    it('renders Category column with correct category badges', async () => {
      ...
      const row1 = screen.getByTestId('capability-row-cap-1');
      expect(within(row1).getByText('Core')).toBeInTheDocument();

      const row2 = screen.getByTestId('capability-row-cap-2');
      expect(within(row2).getByText('Health Check')).toBeInTheDocument();
    });
    ...
    it('shows chevron arrow on each row', async () => {
      renderWithTheme(<CapabilitiesList onSelect={mockOnSelect} refreshKey={0} />);
    ...
    it('searches across type name, description, notes, and category', async () => {
      ...
      const searchInput = screen.getByPlaceholderText('Filter capabilities...');

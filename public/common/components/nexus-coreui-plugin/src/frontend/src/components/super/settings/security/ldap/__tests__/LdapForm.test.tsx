...
describe('LdapForm', () => {
  ...
  it('validates required fields before navigation', async () => {
    ...
    const nextButton = screen.getByRole('button', { name: /Next.*User/i });
  ...
  it('shows Next button on connection step and Save on user/group step', () => {
    ...
    expect(screen.getByRole('button', { name: /Next.*User/i })).toBeInTheDocument();
  ...
  it('shows both steps in step indicator', () => {
    ...
    expect(screen.getByTestId('wizard-step-connection')).toBeInTheDocument();
    expect(screen.getByTestId('wizard-step-userGroup')).toBeInTheDocument();
  });

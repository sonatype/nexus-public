...
/**
 * Main PyPI Search page component.
 *
 * Features:
 * - Search by name, version, summary, keywords, classifiers
 * - Results table showing package info
 * - Click row to navigate to detail page
 * - URL-driven state (bookmarkable)
 */
export function PyPISearchPage({
  onNavigateToDetail,
  initialName = '',
  initialVersion = '',
}: PyPISearchPageProps): JSX.Element {
  const { state, search, loadMore, clear, hasMore } = usePyPISearch();
  ...
  const handleSelect = useCallback((id: string): void => {
    if (onNavigateToDetail) {
      ...
      window.location.hash = `#preview/browse/search/pypi/detail/${encodeURIComponent(id)}`;
    }
  }, [onNavigateToDetail]);
    ...
    <ScrollArea scrollbars="vertical" style={{ height: '100%' }}>
      <Box p="6">
        <Flex direction="column" gap="4">
          {/* Header */}
          <Box>
            <Heading size="6" mb="1">PyPI Search</Heading>
            <Text color="gray">Search for Python packages by name, version, summary, keywords, or classifiers</Text>
          </Box>

          {/* Search Input */}
          <Flex gap="2" align="center">
            <Badge color="yellow" size="2" style={{ flexShrink: 0 }}>PyPI</Badge>
            <Box style={{ flex: 1 }}>
              <TextField.Root
                size="3"

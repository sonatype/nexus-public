...
describe('RolesList', () => {
  ...
  it('should sort by id', async () => {
    ...
    await waitFor(() => {
      const rows = document.querySelectorAll('.entity-table__row');
      // Sorted by ID ascending: custom-role, nx-admin, nx-anonymous
      expect(rows[0]).toHaveTextContent('Custom Role');
      expect(rows[1]).toHaveTextContent('Administrator');
      expect(rows[2]).toHaveTextContent('Anonymous');
    });
  });
  ...
  it('should filter roles by management type when checkbox is selected', async () => {
    renderWithTheme(<RolesList onSelect={mockOnSelect} onCreate={mockOnCreate} />);
    ...
    // Filter by User-Managed (only Custom Role has readOnly: false and is not from external source)
    const userManagedCheckbox = screen.getByRole('checkbox', { name: /User-Managed/i });
    fireEvent.click(userManagedCheckbox);

    await waitFor(() => {
      // Only user-managed roles should show
      expect(screen.getByText('Custom Role')).toBeInTheDocument();
  ...
  it('should have management filter section', async () => {
    renderWithTheme(<RolesList onSelect={mockOnSelect} onCreate={mockOnCreate} />);

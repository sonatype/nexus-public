...
export interface GolangSearchResultsProps {
  results: readonly GolangResult[];
  loading: boolean;
  error?: string;
  totalCount: number;
...
export function GolangSearchResults({
  results,
  loading,
  error,
  totalCount,
  onSelect,
}: GolangSearchResultsProps): JSX.Element {
  if (loading && results.length === 0) {
    return (
      <Flex justify="center" align="center" p="6">
        <Flex direction="column" align="center" gap="3">
          <Spinner size="3" />
          <Text color="gray">Searching Go modules...</Text>
        </Flex>
      </Flex>
    );
  }
  ...
  return (
    <Box>
      <Flex justify="between" align="center" mb="3">
        <Text size="2" color="gray">Showing {results.length} of {totalCount} modules</Text>
      </Flex>
            ...
            <Table.Row key={result.id} onClick={() => onSelect(result.id)} style={{ cursor: 'pointer' }}>
              <Table.Cell>
                <Flex align="center" gap="2">
                  <Globe size={16} />
                  <Text weight="medium">{result.module}</Text>
                </Flex>
              </Table.Cell>
              <Table.Cell>
                <Badge color="cyan" variant="soft">{result.latestVersion}</Badge>
              </Table.Cell>
              <Table.Cell>
                <Text size="2" color="gray">{result.versionCount || '-'}</Text>
              </Table.Cell>
              <Table.Cell>
...
export default GolangSearchResults;

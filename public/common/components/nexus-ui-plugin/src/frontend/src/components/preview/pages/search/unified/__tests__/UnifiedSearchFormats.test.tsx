...
describe('Unified Search - Format-Specific Filters', () => {
  ...
  describe('Ansible Galaxy filters', () => {
    it('should have all Ansible Galaxy-specific filters', () => {
      const filters = getFiltersForFormat('ansiblegalaxy');
      const filterIds = filters.map((f) => f.id);

      expect(filterIds).toContain('repository');
      expect(filterIds).toContain('namespace');
      expect(filterIds).toContain('name');
      // Note: version is a global filter (above search results)
      expect(filterIds).not.toContain('version');
    });

    it('should have correct API parameters for Ansible Galaxy filters', () => {
      const filters = getFiltersForFormat('ansiblegalaxy');
      const filterMap = new Map(filters.map((f) => [f.id, f]));

      expect(filterMap.get('namespace')?.apiParam).toBe('ansible-galaxy.namespace');
      expect(filterMap.get('name')?.apiParam).toBe('ansible-galaxy.name');
    });
  });

    ...
    extends TestSupport
{
  ...
  public void copy() throws IOException {
    ...
    FileHelper.copy(new ByteArrayInputStream(baz.getBytes(Charsets.UTF_8)), target.toPath());
  ...
  public void copyAndOverwrite() throws IOException {
    ...
    final File target = new File(root, "baz.txt");
    FileHelper.copy(new ByteArrayInputStream(baz1.getBytes(Charsets.UTF_8)), target.toPath());
    FileHelper.copy(new ByteArrayInputStream(baz2.getBytes(Charsets.UTF_8)), target.toPath());
    assertThat(target, FileMatchers.isFile());
  ...
  public void readFileNonExistent() throws IOException {
    final File target = new File(root, "baz.txt");
    FileHelper.readFile(target.toPath());
  }
  ...
  public void readFileWriteFileRoundtrip() throws IOException {
    ...
    final File target = new File(root, "baz.txt");
    FileHelper.writeFile(target.toPath(), baz1);
    final String readBaz1 = FileHelper.readFile(target.toPath());
    FileHelper.writeFile(target.toPath(), baz2);
    final String readBaz2 = FileHelper.readFile(target.toPath());
    FileHelper.writeFile(target.toPath(), baz3);
    final String readBaz3 = FileHelper.readFile(target.toPath());

    ...
    extends TestSupport
{
  ...
  public void testWalkFilesWithDifferentDuration() throws Exception {
    ...
    WalkFile walkFile = new WalkFile(temporaryFolder.getRoot().getAbsolutePath(), fiveMin);
    List<String> blobIds = walkFile.getBlobIdUpdatedSinceStream().stream()
        .map(BlobId::toString)
    ...
    walkFile = new WalkFile(temporaryFolder.getRoot().getAbsolutePath(), fiveHours);
    blobIds = walkFile.getBlobIdUpdatedSinceStream().stream()
        .map(BlobId::toString)
        .collect(Collectors.toList());
    ...
    walkFile = new WalkFile(temporaryFolder.getRoot().getAbsolutePath(), fiveDays);
    blobIds = walkFile.getBlobIdUpdatedSinceStream().stream()
        .map(BlobId::toString)
        .collect(Collectors.toList());
    assertThat(blobIds, containsInAnyOrder(
    ...
    walkFile = new WalkFile(temporaryFolder.getRoot().getAbsolutePath(), fiveMonths);
    blobIds = walkFile.getBlobIdUpdatedSinceStream().stream()
        .map(BlobId::toString)
        .collect(Collectors.toList());
    assertThat(blobIds, containsInAnyOrder(
  ...
  private static String getDatePath(final OffsetDateTime blobCreated) {
    return blobCreated.format(DATE_TIME_FORMATTER);
  }
}

...
public class SelectorsSecurityContributorTest
    extends TestSupport
{
  private SelectorsSecurityContributor underTest;

  @Before
  public void setup() {
    underTest = new SelectorsSecurityContributor();
  }
  ...
  public void testGetContribution() {
    ...
    assertThat(config.getPrivileges().stream().map(CPrivilege::getId).collect(toList()),
        containsInAnyOrder("nx-selectors-all", "nx-selectors-create", "nx-selectors-read", "nx-selectors-update",
            "nx-selectors-delete"));
  }
}

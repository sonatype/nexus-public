...
public class AnalyticsSecurityContributorTest
    extends TestSupport
{
  private AnalyticsSecurityContributor underTest;

  @Before
  public void setup() {
    underTest = new AnalyticsSecurityContributor();
  }
  ...
  public void testGetContribution() {
    ...
    assertThat(config.getPrivileges().stream().map(CPrivilege::getId).collect(toList()),
        containsInAnyOrder("nx-analytics-all"));
  }
}

...
public class AtlasSecurityContributorTest
    extends TestSupport
{
  private AtlasSecurityContributor underTest;

  @Before
  public void setup() {
    underTest = new AtlasSecurityContributor();
  }
  ...
  public void testGetContribution() {
    ...
    assertThat(config.getPrivileges().stream().map(CPrivilege::getId).collect(toList()),
        containsInAnyOrder("nx-atlas-all"));
  }
}

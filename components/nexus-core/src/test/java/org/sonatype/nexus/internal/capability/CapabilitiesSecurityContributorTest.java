    ...
    extends TestSupport
{
  ...
  @Before
  public void setup() {
    underTest = new CapabilitiesSecurityContributor();
  }
  ...
  public void testGetContribution() {
    ...
    assertThat(config.getPrivileges().stream().map(CPrivilege::getId).collect(toList()),
        containsInAnyOrder("nx-capabilities-all", "nx-capabilities-create", "nx-capabilities-read",
            "nx-capabilities-update", "nx-capabilities-delete"));
  }
}

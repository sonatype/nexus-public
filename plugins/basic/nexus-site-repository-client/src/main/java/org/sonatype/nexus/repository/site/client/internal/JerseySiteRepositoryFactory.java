...
@Named
@Singleton
public class JerseySiteRepositoryFactory
    extends JerseyHostedRepositoryFactory
{
  ...
  public int canAdapt(final RepositoryBaseResource resource) {
    ...
    if (score > 0) {
      if (JerseySiteRepository.PROVIDER.equals(resource.getProvider())) {
        score++;
      }
    }
    return score;
  ...
  @Override
  public JerseySiteRepository adapt(final JerseyNexusClient nexusClient,
                                    final RepositoryBaseResource resource)
  {
    return new JerseySiteRepository(nexusClient, (RepositoryResource) resource);
  }

  @Override
  public boolean canCreate(final Class<? extends Repository> type) {
    return SiteRepository.class.equals(type);
  }

  @Override
  public JerseySiteRepository create(final JerseyNexusClient nexusClient, final String id) {
    return new JerseySiteRepository(nexusClient, id);
  }

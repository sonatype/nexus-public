    ...
    implements ReplicationIngester
{
  ...
  private final RawReplicationIngesterHelper rawReplicationIngesterHelper;

  @Inject
  public RawReplicationIngester(
      final BlobStoreManager blobstoreManager,
      final RawReplicationIngesterHelper rawReplicationIngesterHelper
  )
  {
    this.blobStoreManager = checkNotNull(blobstoreManager);
    this.rawReplicationIngesterHelper = checkNotNull(rawReplicationIngesterHelper);
  }
      ...
      ReplicationIngestionException
  {
    ...
    if (eventType.equals(BlobEventType.DELETED)) {
      ...
      rawReplicationIngesterHelper.deleteReplication(path, repositoryName);
      return;
    }

    Map<String, Object> backingAssetAttributes = extractAttributesFromProperties(blobAttributes.getProperties());

    try {
      log.info("Ingesting blob {} in repository {} and blob store {}.", blobIdString, repositoryName,
          blobStoreId);
      rawReplicationIngesterHelper.replicate(blobStoreId, blob, backingAssetAttributes, repositoryName, blobStoreId);
    }
    catch (IOException e) {

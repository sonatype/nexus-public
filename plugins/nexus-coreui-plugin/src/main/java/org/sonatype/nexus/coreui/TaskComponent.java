    ...
    implements StateContributor
{
  ...
  public List<TaskTypeXO> readTypes() {
    return taskScheduler.getTaskFactory().getDescriptors().stream().map(this::asTaskTypeXO).collect(toList());
  }
  ...
  private String getSchedule(final Schedule schedule) {
  ...
  private String getLastRunResult(final TaskState endState, final Long runDuration) {
    StringBuilder lastRunResult = new StringBuilder();

    if (endState != null) {
      switch (endState) {
        case OK:
          lastRunResult.append("Ok");
          break;

        case CANCELED:
          lastRunResult.append("Canceled");
          break;

        case FAILED:
          lastRunResult.append("Error");
          break;

        case INTERRUPTED:
          lastRunResult.append("Interrupted");
          break;
  ...
  private TaskTypeXO asTaskTypeXO(final TaskDescriptor taskDescriptor) {
  ...
  private TaskXO asTaskXO(final TaskInfo taskInfo) {
    ...
    if (taskInfo.getCurrentState().getState().isRunning() &&
        taskInfo.getContext().containsKey(EXTENDED_DESCRIPTION_CONTEXT_KEY)) {
      statusDescription += ": " + taskInfo.getContext().get(EXTENDED_DESCRIPTION_CONTEXT_KEY);
    }

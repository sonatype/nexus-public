 ...
 * @since 3.28
 */
@Named(OrientRProxyRecipe.NAME)
@Singleton
class OrientRProxyRecipe
    extends OrientRRecipeSupport
{
  public static final String NAME = 'r-proxy'

  @Inject
  Provider<OrientRProxyFacetImpl> proxyFacet
  ...
  @Inject
  OrientRProxyRecipe(@Named(ProxyType.NAME) final Type type, @Named(RFormat.NAME) final Format format) {
    super(type, format)
  }

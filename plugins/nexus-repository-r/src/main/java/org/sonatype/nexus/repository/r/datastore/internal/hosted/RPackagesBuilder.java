...
public class RPackagesBuilder
{
  ...
  /**
   * Processes an formatAttributes of archive, updating the greatest version and details for the package if
   * appropriate.
   */
  public void append(final NestedAttributesMap formatAttributes) {
    // is this a newer version of this asset's package than the one we currently have (if we have one)?
    String packageName = formatAttributes.get(P_PACKAGE, String.class);
    RPackageVersion oldVersion = packageVersions.get(packageName);
    RPackageVersion newVersion = new RPackageVersion(formatAttributes.get(P_VERSION, String.class));
    if (oldVersion == null || newVersion.compareTo(oldVersion) > 0) {
      ...
      newInformation.put(P_PACKAGE, formatAttributes.get(P_PACKAGE, String.class));
      newInformation.put(P_VERSION, formatAttributes.get(P_VERSION, String.class));
      newInformation.put(P_DEPENDS, formatAttributes.get(P_DEPENDS, String.class));
      newInformation.put(P_IMPORTS, formatAttributes.get(P_IMPORTS, String.class));
      newInformation.put(P_SUGGESTS, formatAttributes.get(P_SUGGESTS, String.class));
      newInformation.put(P_LINKINGTO, formatAttributes.get(P_LINKINGTO, String.class));
      newInformation.put(P_LICENSE, formatAttributes.get(P_LICENSE, String.class));
      newInformation.put(P_NEEDS_COMPILATION, formatAttributes.get(P_NEEDS_COMPILATION, String.class));
